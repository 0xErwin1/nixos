...
**Recursive directory targets** (`.pi/agent/skills`, `.config/opencode/{agent,command,commands,skills}`, `.claude/{agents,commands,skills}`, `.codex/skills`, `.grok/agents`, etc.) are materialized as a real directory whose leaf files are the managed symlinks — the directory itself is never a symlink. The preflight therefore skips them; checking the directory's top level would abort on every switch after the first. Genuine leaf-file collisions inside these directories are still caught by Home Manager's own file-collision detection.
