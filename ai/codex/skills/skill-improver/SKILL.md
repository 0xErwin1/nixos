...
- Read `references/skill-style-guide.md` first and treat it as the normative style contract.
- Treat `SKILL.md` as the source of truth; preserve author intent, critical rules, activation semantics, and output requirements.
- Use `.agents/skill-registry.md` as an index of skill names, triggers, scopes, and exact paths when available.
...
1. Read `references/skill-style-guide.md`.
2. Read `.agents/skill-registry.md`; use listed paths to select skills. If missing, scan known skill directories for `*/SKILL.md`.
