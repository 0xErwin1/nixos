...
0. GUIDED INTAKE — if $ARGUMENTS is empty, vague, or does not name a concrete feature/flow/screen/endpoint:
   ...
   4. **How should I drive it?** — present the two personas in plain language, NOT jargon:
      - "Dejo tests automáticos guardados en el código para repetir después." → `playwright (code)` / `playwright`
      - "Reviso en vivo desde un dispositivo, emulador, simulador, o web/Chromium con Maestro (sin código salvo que autorices guardar flujos)." → `maestro (visual device)` / `maestro`
      Recommend the second option for no-code/live visual validation and mobile/device-first validation. This is the persona engine — it is BLOCKING per the Testing Setup Questions; do not assume it.
...
Before starting, search for prior testing work on this feature:
  mem_search(query: "testing/{project_slug}/{feature_slug}", project: "{project}")
