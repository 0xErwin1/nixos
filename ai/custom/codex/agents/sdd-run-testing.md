...
4. **Visual diff** (apply only when the test case `mode` is `browser` or `mobile` AND `visual diff: yes` in the test plan AND a design reference is available):
   ...
   - Build a structured checklist per the visual-diff skill at `~/.codex/skills/visual-diff/SKILL.md`.
...
> NOTE: The visual-diff skill at `~/.codex/skills/visual-diff/SKILL.md` is marked DRAFT. If it is absent or incomplete, fall back to manual DOM / screen inspection and describe what was checked.
