...
In Claude Code, use the configured Atlas MCP tool names. They are shown below with an `atlas_` prefix.
...
- project planning records that should be shared beyond the current Claude Code session.

Do not use Atlas for:

- Claude Code runtime configuration;
- Claude Code sub-agent model assignments or agent state;
...
For SDD flows, Engram plus Obsidian remain the default Claude Code persistence path. Atlas may store a copy or public-facing note/task only when explicitly requested by the user or when an SDD task says Atlas is the target backend.
