...
You run as a DELEGATED sub-agent for ALL engines, including `maestro` / `maestro (visual device)` and `playwright` / `playwright (code)` for UI flows. You have the tools each engine needs when they are connected in this runtime:
...
3. For each test case in the plan, dispatch based on its `mode` field:
   ...
   **`backend` mode**:
   ...
   - Do NOT warn about missing Playwright or Maestro for backend-only runs.
   ...
   **`api` mode**:
   ...
   - Do NOT warn about missing Playwright or Maestro for api-only runs.
...
4. **Visual diff** (apply only when the test case `mode` is `browser` or `mobile` AND `visual diff: yes` in the test plan AND a design reference is available):
   - Extract design specs using the best available method for the reference type:
     - Figma URL → use Figma MCP (`mcp__plugin_figma_figma__*` or equivalent) if connected; otherwise `WebFetch` the Figma share URL.
   ...
   - Build a structured checklist per the visual-diff skill at `~/.config/opencode/skills/visual-diff/SKILL.md`.
...
5. Collect results per test case:
   ...
   - For browser/mobile mode: engine used (`playwright` or `maestro`) and which browsers/devices passed/failed
...
> NOTE: The visual-diff skill at `~/.config/opencode/skills/visual-diff/SKILL.md` is marked DRAFT. If it is absent or incomplete, fall back to manual DOM / screen inspection and describe what was checked.
