...
TASK:
Resolve the active SDD project with Engram `mem_current_project` and pass its returned project value unchanged to the configured hidden `sdd-onboard` agent with the working directory, artifact-store mode, and complete preflight decision block. Tell it to read `~/.config/opencode/skills/sdd-onboard/SKILL.md` and `~/.config/opencode/skills/_shared/sdd-phase-common.md` before acting.
