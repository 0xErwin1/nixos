...
When user input requires a choice, retain every question, option, default, consequence, and answer syntax in the session decision state. If the `question` tool is unavailable or cannot represent the complete choice, present the full equivalent envelope in chat, block for an answer, and do not infer or silently discard a choice.
...
**Composition.** Batching governs apply EXECUTION checkpoints only. How the finished work is delivered — one PR, several, or none — is decided by the user outside SDD and never constrains batch boundaries.
