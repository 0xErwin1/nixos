...
Ask all three preflight groups in one single grouped call so the client renders them as one interactive prompt. Do NOT run this as a sequential wizard. Do NOT issue three separate calls.

The single grouped call must contain these three localized groups in this order:
...
Apply is still batched into checkpoints when the change is large — that is a separate mechanism driven by task count and phase boundaries, for context freshness and rollback granularity. Never size an apply batch to fit the review budget.
