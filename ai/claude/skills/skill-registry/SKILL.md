...
- Always write `.agent/skill-registry.md` regardless of SDD persistence mode.
...
- Add `.agent/` to `.gitignore` when possible unless explicitly disabled.
...
3. Render `.agent/skill-registry.md` with scanned sources, registry contract, skill name, trigger/description, scope, and exact path.
...
Return:
- Project name and `.agent/skill-registry.md` path.
