...
5. Check browser / Chrome-extension readiness when that path matters:
   - `command -v google-chrome || command -v chromium || command -v chrome`
   - Claude CLI / extension bridge availability if the user wants the Chrome-extension path
...
- whether the Chrome-extension path still needs a manual browser-side step
