...
- The stack has changed (new auth flow, new Playwright config location, new environments).
...
| `TESTING_SETUP.md` | Generated always | Technical setup: how to run tests, auth, environments, test data, known flaky areas |
...
The split is intentional: `TESTING_CONTEXT.md` is for *understanding results* — product constraints, business rules, non-obvious behaviors, and design references. `TESTING_SETUP.md` is for *running tests* — commands, credentials, environments, test data, and known flaky areas (operational).
...
- Test script names from `package.json` scripts
- `.env.example` or `.env.test` for env var names (never log values)
- Framework detection (Next.js, Rails, Django, etc.) for backend test command hints
- `package.json` dependencies for test libraries (vitest, jest, cypress, playwright)
