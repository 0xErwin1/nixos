...
The orchestrator is self-sufficient. It MUST NOT assume `CLAUDE.md`, a persona file, or any other instruction file is co-loaded. Every behavior required by the orchestrator — including persistent memory — is inlined below.
...
<!-- tabularium-ai:trigger-rules -->

### Agent Trigger Rules (4R review lenses)

These are organic recommendations, not enforced checkpoints. The orchestrator decides when to act on them using its own judgment; nothing here gates a phase, a commit, or a PR.

The four read-only review lenses are the `review-risk`, `review-readability`, `review-reliability`, and `review-resilience` agents (R1-R4). Each reports findings only and never fixes code.

- At **pre-commit**, consider running `review-readability` -- ONE cheap advisory lens for an everyday event. Reserve the full 4R fan-out for pre-PR.
- At **pre-PR**, **strongly recommend** running all four 4R lenses (`review-risk`, `review-resilience`, `review-readability`, and `review-reliability`) in parallel WHEN the diff touches sensitive paths (`**/auth/**`, `**/update/**`, `**/security/**`, `**/payments/**`) OR exceeds ~400 changed lines. Outside those conditions the full fan-out is usually not worth its cost; use judgment.

This is agent-judgment guidance, not a hard gate. It composes with the Review Workload Guard and the Automatic Mode Gatekeeper without relaxing either.
...
The Claude Code session model is controlled by Claude Code itself; this asset does not configure the main orchestrator model. This table applies only to Agent tool calls for SDD phase sub-agents and general delegation.
