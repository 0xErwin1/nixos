...
Otherwise, read the agent file at `~/.claude/agents/sdd-report-testing.md` FIRST, then follow its instructions exactly inline.
...
Read run results (required):
  mcp__plugin_engram_engram__mem_search(query: "testing/{project_slug}/$ARGUMENTS/run/latest", project: "{project}") → mem_get_observation(id)
Read plan (required):
