---
description: Investigate a feature or flow for testing — reads codebase, Figma, and ClickUp context
---

If the native `sdd-explore-testing` sub-agent is available, delegate this command to it.
Otherwise, read the agent file at `~/.claude/agents/sdd-explore-testing.md` FIRST, then follow its instructions exactly inline.
...
Read project context (optional):
  mcp__plugin_engram_engram__mem_search(query: "sdd-init/{project}", project: "{project}") → if found, mem_get_observation(id)
