...
0. GUIDED INTAKE — if $ARGUMENTS is empty, vague, or does not name a concrete feature/flow/screen/endpoint:
   ...
   4. **How should I drive it?** — present the two personas in plain language, NOT jargon:
      - "Reviso en vivo, abriendo el navegador yo mismo (no escribo código, no toco el repo)." → `live (no code)`
      - "Dejo tests automáticos guardados en el código para repetir después." → `playwright (code)`
      Recommend the first option for non-technical users. This is the persona engine — it is BLOCKING per
      CLAUDE.md Testing Setup Questions; do not assume it.
   ...
   The wizard answers SATISFY the corresponding CLAUDE.md Testing Setup Questions — do NOT re-ask persona
   engine or target environment afterward. Still resolve the remaining setup questions the wizard did not
   cover (test-case source and execution mode, step 3 below), in the same plain language, only if not
...
2. Run the two prerequisites checkpoints defined in the CLAUDE.md `## SDD Testing Workflow → ### Prerequisites Check` section. Checkpoint 1 runs before explore-testing; Checkpoint 2 runs after plan-testing returns (once modes and engines are known), before run-testing.
...
4. After explore-testing returns and BEFORE launching plan-testing: resolve the test-case suites and
   get the user's sign-off per CLAUDE.md `### Test-Case Source → Suites resolution gate` BEFORE launching
...
5. Run each phase as a sub-agent in sequence, pausing between phases in interactive mode. `run-testing` is
   delegated for every engine, including `chrome-extension` / `live (no code)` — the sub-agent drives the
   Chrome extension directly. It is fanned out into N parallel runners, one per execution unit from the
   plan (independent cases run concurrently; dependency chains stay ordered in one runner); the orchestrator
   merges the shards. See CLAUDE.md `### Execution Policy → run-testing is always delegated` and
   `→ Parallel execution`.
