...
5. Check browser / web-surface readiness when Playwright or Maestro web validation matters:
   ...
   - Claude CLI / extension bridge availability if the user wants the `chrome-extension` engine (Claude Code only, when the Claude Chrome extension is available)
   - any repo-documented browser launch command or approved web target for Maestro (the web-surface path on other clients)
...
- Browser / web-surface path (Claude Code: `chrome-extension` engine when available; other clients: repo-documented browser launch or approved web target)
...
- whether the browser / web-surface path (chrome-extension on Claude Code, or any other browser/web target) still needs a manual start, login, or approval step
