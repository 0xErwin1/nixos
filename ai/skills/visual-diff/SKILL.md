...
- `chrome-extension` — real Chrome session, Chrome-only visual verification (Claude Code only, when the Claude Chrome extension is available)
...
`engine: maestro` is the device / Chromium path for browser or mobile validation; `engine: chrome-extension` is real-session Chrome only, and is available on Claude Code only.
...
- **Figma URL or node ID**: use the configured Figma MCP tools if connected (for example `mcp__claude_ai_Figma__get_design_context` or `mcp__plugin_figma_figma__*`, depending on the client). If the MCP is absent, fetch the Figma share URL with `WebFetch` and extract any readable spec.
...
- **`engine: chrome-extension`** (Claude Code only, when the Claude Chrome extension is available) → drive the live Chrome session via the Claude Chrome extension; read computed styles through the extension's DOM inspection / element evaluation and capture a screenshot from the live tab. Do NOT use Playwright for this engine.
...
- **Informative screenshot / evidence** — persistence depends on the persona engine (the screenshot is informative-only and never gates pass/fail):
  ...
  - `live (no code)` / `chrome-extension` (Claude Code only): do NOT write any file to the repo. Describe the screenshot in the run artifact (and attach it to the engram run observation if supported).
