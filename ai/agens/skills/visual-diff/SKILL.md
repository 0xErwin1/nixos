...
- **Figma URL or node ID**: use the Figma MCP (`mcp__claude_ai_Figma__get_design_context` or equivalent) if connected. If the MCP is absent, fetch the Figma share URL with `WebFetch` and extract any readable spec.
